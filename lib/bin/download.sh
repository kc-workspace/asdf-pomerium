...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    url="${ASDF_OVERRIDE_REF_REPO:-https://github.com/pomerium/cli.git}"
    ...
    url="https://github.com/pomerium/cli/releases/download/v{version}/pomerium-cli-{os}-{arch}.tar.gz"
    url="$(kc_asdf_template "$url" "${vars[@]}")"
  ...
  if [[ "$mode" == "custom" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://github.com/pomerium/cli/releases/download/v{version}/pomerium-cli_checksums.txt"
